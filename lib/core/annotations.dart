...
///  - with parameters: [ActionMethodPreProcessorContext] followed by
///    zero or more parameters with the same type of the
///    [ServiceObjectActionMethod] that it supports.
///
///  The [ReflectCodeGenerator] will look for all the
///  [ActionMethodParameterProcessor]s.
///  These will be used to generate [ActionMethodInfo]s.
///
/// See the functions in default_action_method_parameter_processor.dart
/// for the default implementations and inspiration.
class ActionMethodParameterProcessor {
  /// A number to set the index compared to other [ActionParameterAction]s.
  /// This becomes important if multiple [ActionParameterAction]s
  /// handle the same [Type]s and annotations
  /// A double is used so that there are endless numbers to put between to
  /// existing numbers.
  final double index;

  /// If the [ActionMethod] must have a [ProcessDirectly] annotation,
  /// in order to be processed by the [ActionMethodParameterProcessor].
  /// Default=false
...
enum ExecutionMode {
  /// Execute the [ActionMethod] directly, by calling
  /// [ActionMethodInfo.invokeMethodAndProcessResult] in
  /// the [ActionMethodInfo.start] method
  directly,

  /// First show the [ActionMethod] parameter value and ask the user to to
  /// confirm (e.g. with a dialog).
  /// If so, call [ActionMethodInfo.invokeMethodAndProcessResult]
  firstAskConformation,

  /// First let the user edit the [ActionMethod] parameter value
  /// (e.g. in a [FormTab]).
  /// Then, call [ActionMethodInfo.invokeMethodAndProcessResult]
  firstEditParameter
}
...
///  - with return type void
///  - with parameters: [ActionMethodPreProcessorContext] followed by
///    zero or more parameters with the same type of the [ActionMethod]
///    that it supports.
///
///  The [ReflectCodeGenerator] will look for all the
///  [ActionMethodResultProcessor]s. These will be used to
///  generate [ActionMethodInfo]s.
///
/// See the  functions in default_action_method_result_processor.dart
/// for the default implementations and inspiration.

class ActionMethodResultProcessor {
  /// A number to set the index compared to other [ActionMethodResultProcessor]s.
  /// This becomes important if multiple [ActionParameterAction]s handle the
  /// same [Type]s and annotations.
  /// A double is used so that there are endless numbers to put between to
  /// existing numbers.
  final double index;
...
/// Annotation to indicate that a class is a [ServiceObject], so that they are
/// recognized by the [ReflectFramework]
/// See [ServiceClassInfo]
class ServiceClass {
  const ServiceClass();
}

/// The [DomainClass] annotation is only used by the [ReflectCodeGeneration]
/// to indicate that a method parameter or return value is a [DomainObject].
/// See:
/// - [ActionMethodParameterProcessor]s
...
/// A [ActionMethodParameterFactory] creates an new instance for the
/// [ActionMethod] parameter.
/// This parameter can than be processed (See [ActionMethodParameterProcessor])
/// after which it is passed as the [ActionMethodParameter] parameter when the
/// [ActionMethod] is invoked bij the [ActionMethodResultProcessor].
/// The MainMenu will display all [ActionMethod]s of all registered
/// [ServiceClass]es that can directly be executed.
/// This means that only [ActionMethod]s that either have no method parameter
/// or have an ParameterFactory are displayed as menu items in the MainMenu.
///
/// You can put an [ActionMethodParameterFactory] annotation before an
/// [ActionMethod] to indicate that the parameter can be created with an
...
/// The key will be the library name + annotation path + suffix.
/// All words will be in lower camelCase and all words will be separated with dots.
///
...
/// }
class Translation {
  /// If keySuffix=null: than the translation refers to a
  /// library member (Class, Method, Property)
  /// If keySuffix=(a camelCase string): than the translation refers to a
  /// translatable string that could be used in your dart code
